...
export type IntNotificationType =
  | "like"
  | "comment"
  | "follow"
  | "mention"
...
export type IntNotification = {
  id: string;
  type: IntNotificationType;
  actor: {
    id: string;
    name: string;
    avatar: string;
...
export const IntNotifications: IntNotification[] = [
  {
    id: "n1",
    type: "like",
    actor: {
...
const LazyIntNotifDescription = dynamic(() => import("./Desc"), {
  loading: () => <p>Loading…</p>,
});

interface IntNotificationPageProps {
  params: Promise<{ slug: string }>;
  searchParams: Promise<{id?: string}>
}

const IntNotificationPage = async ({ params, searchParams }: IntNotificationPageProps) => {
  const url = (await params).slug;
  const idParams = (await searchParams).id
  ...
  return (
    <>
      {url === "notification" && (
        <LazyIntNotifDescription data={IntNotifications} idParams={idParams}/>
      )}
    </>
  );
};

export default IntNotificationPage;
